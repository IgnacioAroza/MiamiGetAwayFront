...
const AdminDashboard = () => {
  ...
  const stats = useMemo(() => ({
    ...
    totalRevenue: payments ? payments.reduce((total, payment) => {
      const amount = parseFloat(payment?.amount || 0);
      return total + (isNaN(amount) ? 0 : amount);
    }, 0) : 0
  }), [totalUsers, apartments.length, reservations?.length, payments, reviews?.length]);
                    ...
                    latestReservations.map((reservation) => (
                      <ListItem key={reservation.id} sx={{ 
                        ...
                        <ListItemText
                          ...
                          secondary={`${reservation.client_name || 'Client'}, ${reservation.client_lastname || 'Lastname'} · ${formatDate(reservation.check_in_date)} to ${formatDate(reservation.check_out_date)}`}
                        />
                      </ListItem>
                    ))
                  )}
